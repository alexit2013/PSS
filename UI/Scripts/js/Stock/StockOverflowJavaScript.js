...
$(function () {
    ...
    $("#probodyed").delegate("tr", "click", function () {
        ...
        $.post("/Stock/PayOffs/GetDeteilByID", { id: stockid }, function (data) {
            if (data) {
                $("#stockbodyed tr").remove();
                $(data).each(function (k, v) {
                    var tr = $("<tr></tr>");
                    var td1 = $("<td>" + v.ProName + "</td>");
                    var td2 = $("<td>" + v.PODPrice + "</td>");
                    var td3 = $("<td>" + v.PODAmount + "</td>");
                    var td4 = $("<td><i style='color:green;cursor:pointer;font-size:16px;' class='am-icon-comment'  title='" + v.PODDesc + "'></i></td>");
    ...
    $("#btn_addorder").click(function () {
        ...
        var PODesc = $("#PODesc").text();
        $("#orderbody tr").each(function (k, v) {
            var tdid = $(v).children("td")[0].innerHTML;
            if (!isNaN(tdid)) {
                var PODAmount = $($(v).children("td")[5]).children("input")[0].value;//数量
                var PODPrice = $($(v).children("td")[6]).children("input")[0].value;//价格
                var PODDesc = $($(v).children("td")[8]).children("input")[0].value;//描述
                var st = { POID: POID, ProID: tdid, PODAmount: PODAmount, PODPrice: PODPrice, PODDesc: PODDesc };
                list.push(st);
            }
        });
        var stock = { POID: POID, DepotID: DepotID, PODate: PODate, POState: 0, PODesc: PODesc };
        //alert(JSON.stringify(list));
        //alert(JSON.stringify(stock));
        var type = $(this).val();
        if (type == "保存") {
            $.post("/Stock/PayOffs/Add", { pd: stock, list: list }, function (data) {
                if (data == "add_yes") {
            ...
            $.post("/Stock/PayOffs/Edit", { pd: stock, list: list }, function (data) {
                if (data == "edit_yes") {
...
function Page() {
    ...
    var PODate = $("#PODate1").val();
    var UsersName = $("#UsersName1").val();
    var POState = $("input[name=POState1]:checked").val();
    var obj = { POID: POID, UsersName: UsersName, DepotID: DepotID, PODate: PODate, POState: POState, PageIndex: PageIndex };
    // alert(JSON.stringify(obj));
    $.post("/Stock/PayOffs/Find", obj, function (data) {
        if (data) {
...
function setPage(data) {
    ...
    $(data).each(function (k, v) {
        var tr = $("<tr></tr>");
        var td1 = $("<td>" + v.POID + "</td>");
        var td2 = $("<td>" + v.DepotName + "</td>");
        var td3 = $("<td></td>");
        if (v.PODate != "null" && v.PODate != "undefined") {
            td3.text(ConvertTime(v.PODate));
        }
        ...
        var td6 = $("<td ><i style='color:green;cursor:pointer;font-size:16px;' class='am-icon-comment'  title='" + v.PODesc + "'></i></td>");
        var td7 = $("<td align='center'><button style='font-size:10px;' class='am-btn am-btn-warning am-radius   am-btn-xs '  " + (v.POState == "1" ? "disabled='disabled'" : "") + "  onclick='orderedit(\"" + v.POID + "\",\"" + v.DepotID + "\",\"" + v.UserID + "\",\"" + v.PODate + "\",\"" + v.POState + "\",\"" + v.PODesc + "\")'><span class='am-icon-pencil-square-o'></span> 编辑</button><button style='font-size:10px;margin-left:3px;' class='am-btn am-btn-success am-radius am-btn-xs' " + (v.POState == "1" ? "disabled='disabled'" : "") + "  onclick='order_ck(\"" + v.POID + "\")' ><span class='am-icon-check'></span> 审核</button> <button style='font-size:10px;' class='am-btn am-btn-danger am-radius am-btn-xs' " + (v.POState == "1" ? "disabled='disabled'" : "") + " onclick='order_del(\"" + v.POID + "\")' ><span class='am-icon-trash-o'></span> 删除</button></td>");
...
function order_del(id) {
    new tmm_confirm("del", "提示", "确认要删除吗？", function () {
        $.post("/Stock/PayOffs/Del", { id: id }, function (data) {
            if (data == "del_yes") {
...
//编辑入库单
function orderedit(POID, DepotID, UserID, PODate, POState, PODesc) {
    $("#btn_addorder").val("修改");
    ...
    $("#POID").val(POID);
    $("#PODate").val(ConvertTime_(PODate));
    $("#PODesc").val(PODesc);
    $("#DepotID option[value='" + DepotID + "']").prop("selected", true);
    //显示商品信息
    $.post("/Stock/PayOffs/GetDeteilByID", { id: POID }, function (data) {
        $("#orderbody tr").remove();
        if (data) {
            $(data).each(function (k, v) {
                var newtr = $('<tr><td align="center">' + v.ProID + '</td><td>' + v.ProName + '</td><td>' + v.PUName + '</td><td>' + v.PSName + '</td><td>' + v.PCName + '</td><td ><input  type="number" name="procount" value="' + v.PODAmount + '" placeholder="请输入商品数量" style="width:100px;font-size:11px;" required/></td><td><input type="number" value="' + v.PODPrice + '"  name="proprice"   placeholder="请输入商品单价" style="width:100px;font-size:11px;" required /></td><td>' + (v.PODAmount * v.PODPrice).toFixed(2) + '</td><td><input type="text" value="' + v.PODDesc + '" placeholder="备注"  style="width:160px;font-size:11px;"  /></td><td align="center"><a class="am-icon-remove" title="删除" href="#" style="font-size:18px;"></a></td></tr>');
                $("#orderbody").append(newtr);
            });
        }
    });
...
function order_ck(id) {
    new tmm_confirm("ck", "提示", "确认操作？", function () {
        $.post("/Stock/PayOffs/CKOtherDepot", { id: id }, function (data) {
            if (data == "ck_yes") {
...
//获取ID
function getid() {
    $.post("/Stock/PayOffs/GetID", {}, function (data) {
        if (data) {
            $("#POID").val(data);
        } else {
            $("#POID").val("BY201501010001");
        }
    });
}
//选择客户时
