...
$(function () {
    ...
    $("#probodyed").delegate("tr", "click", function () {
        ...
        $.post("/InStock/StockInDepot/GetPageStocks", { PageIndex: PageIndex1, SDID: stockid }, function (data) {
            if (data) {
                $("#stockbodyed tr").remove();
                $(data).each(function (k, v) {
                    var tr = $("<tr></tr>");
                    var td1 = $("<td>" + v.SDetailID + "</td>");
                    var td2 = $("<td>" + v.ProName + "</td>");
                    var td3 = $("<td>" + v.SDetailAmount + "</td>");
                    var td4 = $("<td>" + v.SDetailPrice + "</td>");
                    var td5 = $("<td>" + v.SDetailDepAmount + "</td>");
                    var td6 = $("<td><i style='color:green;cursor:pointer;font-size:16px;' class='am-icon-comment'  title='" + v.SDetailDesc + "'></i></td>");
    ...
    $("#btn_addorder").click(function () {
        ...
        $("#orderbody tr").each(function (k,v) {
            ...
            if (!isNaN(tdid)) {
                var SDetailAmount = $($(v).children("td")[5]).children("input")[0].value;
                var SDetailPrice = $($(v).children("td")[6]).children("input")[0].value;
                var SDetailDesc = $($(v).children("td")[8]).children("input")[00].value;
                var st = { PPID: PPID, ProID: tdid, StockID: StockID, SDetailAmount: SDetailAmount, SDetailPrice: SDetailPrice, SDetailDepAmount: SDetailPrice, SDetailDesc: SDetailDesc };
                list.push(st);
            }
        });
        var stock = { StockID: StockID, PPID: PPID, StockDate: StockDate, StockInDate: StockInDate, StockState: 0, StockDesc: StockDesc };
        //alert(JSON.stringify(stock));
        //alert(JSON.stringify(list));
        var type = $(this).val();
        if (type == "保存") {
            $.post("/InStock/StockInDepot/AddStocks", { st: stock, list: list }, function (data) {
            if(data=="add_yes"){
            ...
            $.post("/InStock/StockInDepot/EditStocks", { st: stock, list: list }, function (data) {
                if (data == "edit_yes") {
                    new tmm_alert1("edit_modal", "提示", "添加成功");
                    $("#btn_addorder").val("保存");
                    ...
                    new tmm_alert("edit_modal", "提示", "添加失败");
                } else {
...
function Page() {
    ...
    $.post("/InStock/StockInDepot/StockSPage", { PageIndex: PageIndex }, function (data) {
        if (data) {
            setPage(data);
        }
    });
...
function setPage(data) {
    ...
    $(data).each(function (k, v) {
        ...
        var td8 = $("<td align='center'><button style='font-size:10px;' class='am-btn am-btn-warning am-radius   am-btn-xs '  " + (v.StockState == "1" ? "disabled='disabled'" : "") + "  onclick='orderedit(\"" + v.StockID + "\",\"" + v.PPID + "\",\"" + v.StockDate + "\",\"" + v.StockInDate + "\",\"" + v.StockUser + "\",\"" + v.StockState + "\",\"" + v.StockDesc + "\")'><span class='am-icon-pencil-square-o'></span> 编辑</button><button style='font-size:10px;margin-left:3px;' class='am-btn am-btn-success am-radius am-btn-xs' " + (v.StockState == "1" ? "disabled='disabled'" : "") + "  onclick='order_ck(\"" + v.StockID + "\")' ><span class='am-icon-check'></span> 审核</button> <button style='font-size:10px;' class='am-btn am-btn-danger am-radius am-btn-xs' onclick='order_del(\"" + v.StockID + "\")' ><span class='am-icon-trash-o'></span> 删除</button></td>");
...
function order_del(id) {
  new  tmm_confirm("del", "提示", "确认要删除吗？", function () {
      $.post("/InStock/StockInDepot/DelStocks", { id: id }, function (data) {
          if (data == "del_yes") {
...
//编辑采购单
function orderedit(StockID, PPID, StockDate, StockInDate, StockUser, StockState, StockDesc) {
    $("#btn_addorder").val("修改");
    ...
    $("#StockID").val(StockID);
    $("#StockDate").val(ConvertTime_(StockDate));
    $("#StockInDate").val(ConvertTime_(StockInDate));
    $("#StockDesc").val(StockDesc);
    $("#PPID option[value='" + PPID + "']").prop("selected","selected");
    //显示商品信息
...
function order_ck(id) {
    new tmm_confirm("ck", "提示", "确认操作？", function () {
        $.post("/InStock/StockInDepot/CKStocks", { id: id }, function (data) {
            if (data == "ck_yes") {
...
//获取ID
function getid() {
    $.post("/InStock/StockInDepot/GetStocksID", {}, function (data) {
        if (data) {
            $("#StockID").val(data);
        } else {
            $("#StockID").val("GG201501010001");
        }
    });
}