...
$(function () {
    ...
    $("#probodyed").delegate("tr", "click", function () {
        ...
        $.post("/InStock/OtherInDepot/GetOtherDepotDeteilByID", { id: stockid }, function (data) {
            if (data) {
                $("#stockbodyed tr").remove();
                $(data).each(function (k, v) {
                    var tr = $("<tr></tr>");
                    var td1 = $("<td>" + v.OIDDID + "</td>");
                    var td2 = $("<td>" + v.ProName + "</td>");
                    var td4 = $("<td>" + v.OIDDPrice + "</td>");
                    var td5 = $("<td>" + v.OIDDAmount + "</td>");
                    var td6 = $("<td><i style='color:green;cursor:pointer;font-size:16px;' class='am-icon-comment'  title='" + v.OIDDDesc + "'></i></td>");
    ...
    $("#btn_addorder").click(function () {
        ...
        var OIDDate = $("#OIDDate").val();
        var OIDDesc = $("#OIDDesc").text();
        $("#orderbody tr").each(function (k, v) {
            var tdid = $(v).children("td")[0].innerHTML;
            if (!isNaN(tdid)) {
                var OIDDAmount = $($(v).children("td")[5]).children("input")[0].value;
                var OIDDPrice = $($(v).children("td")[6]).children("input")[0].value;
                var OIDDDesc = $($(v).children("td")[8]).children("input")[00].value;
                var st = { OIDID: OIDID, ProID: tdid, OIDDAmount: OIDDAmount, OIDDPrice: OIDDPrice, OIDDDesc: OIDDDesc };
                list.push(st);
            }
        });
        var stock = { OIDID: OIDID, DepotID: DepotID, OIDDate: OIDDate, OIDState: 0, OIDDesc: OIDDesc };
        //alert(JSON.stringify(list));
      //  alert(JSON.stringify(stock));
        var type = $(this).val();
        if (type == "保存") {
            $.post("/InStock/OtherInDepot/AddOtherDepot", { pd: stock, list: list }, function (data) {
                if (data == "add_yes") {
            ...
            $.post("/InStock/OtherInDepot/EditOtherInDepot", { pd: stock, list: list }, function (data) {
                if (data == "edit_yes") {
...
///加载采购订单数据【分页】
function Page() {
    var OIDID = $("#PIDID1").val();
    var DepotID = $("#DepotID1 option:selected").val();
    var OIDDate = $("#PIDDate1").val();
    var UsersName = $("#UsersName1").val();
    var OIDState = $("input[name=PIDState1]:checked").val();
    var obj = { OIDID: OIDID, UsersName: UsersName, DepotID: DepotID, OIDDate: OIDDate, OIDState: OIDState, PageIndex: PageIndex };
    $.post("/InStock/OtherInDepot/Find", obj, function (data) {
        if (data) {
...
function setPage(data) {
    ...
    $(data).each(function (k, v) {
        var tr = $("<tr></tr>");
        var td1 = $("<td>" + v.OIDID + "</td>");
        var td2 = $("<td>" + v.DepotName + "</td>");
        var td3 = $("<td></td>");
        if (v.OIDDate != "null" && v.OIDDate != "undefined") {
            td3.text(ConvertTime(v.OIDDate));
        }
        ...
        var td6 = $("<td ><i style='color:green;cursor:pointer;font-size:16px;' class='am-icon-comment'  title='" + v.OIDDesc + "'></i></td>");
        var td7 = $("<td align='center'><button style='font-size:10px;' class='am-btn am-btn-warning am-radius   am-btn-xs '  " + (v.OIDState == "1" ? "disabled='disabled'" : "") + "  onclick='orderedit(\"" + v.OIDID + "\",\"" + v.DepotID + "\",\"" + v.OIDDate + "\",\"" + v.OIDUser + "\",\"" + v.OIDState + "\",\"" + v.OIDDesc + "\")'><span class='am-icon-pencil-square-o'></span> 编辑</button><button style='font-size:10px;margin-left:3px;' class='am-btn am-btn-success am-radius am-btn-xs' " + (v.OIDState == "1" ? "disabled='disabled'" : "") + "  onclick='order_ck(\"" + v.OIDID + "\")' ><span class='am-icon-check'></span> 审核</button> <button style='font-size:10px;' class='am-btn am-btn-danger am-radius am-btn-xs' " + (v.OIDState == "1" ? "disabled='disabled'" : "") + " onclick='order_del(\"" + v.OIDID + "\")' ><span class='am-icon-trash-o'></span> 删除</button></td>");
...
function order_del(id) {
    new tmm_confirm("del", "提示", "确认要删除吗？", function () {
        $.post("/InStock/OtherInDepot/DelOtherDepot", { id: id }, function (data) {
            if (data == "del_yes") {
...
//编辑入库单
function orderedit(OIDID, DepotID, OIDDate, OIDUser, OIDState, OIDDesc) {
    $("#btn_addorder").val("修改");
    ...
    $("#OIDID").val(OIDID);
    $("#OIDDate").val(ConvertTime_(OIDDate));
    $("#OIDDesc").val(OIDDesc);
    $("#DepotID option[value='" + DepotID + "']").prop("selected", true);
    //显示商品信息
    $.post("/InStock/OtherInDepot/GetOtherDepotDeteilByID", { id: OIDID }, function (data) {
        $("#orderbody tr").remove();
        if (data) {
            $(data).each(function (k, v) {
                var newtr = $('<tr><td align="center">' + v.ProID + '</td><td>' + v.ProName + '</td><td>' + v.PUName + '</td><td>' + v.PSName + '</td><td>' + v.PCName + '</td><td ><input  type="number" name="procount" value="' + v.OIDDAmount + '" placeholder="请输入商品数量" style="width:100px;font-size:11px;" required/></td><td><input type="number" value="' + v.OIDDPrice + '"  name="proprice"   placeholder="请输入商品单价" style="width:100px;font-size:11px;" required /></td><td>￥' + v.OIDDAmount * v.OIDDPrice + '</td><td><input type="text" value="' + v.OIDDDesc + '" placeholder="备注"  style="width:160px;font-size:11px;"  /></td><td align="center"><a class="am-icon-remove" title="删除" href="#" style="font-size:18px;"></a></td></tr>');
                $("#orderbody").append(newtr);
            });
        }
    });
...
function order_ck(id) {
    new tmm_confirm("ck", "提示", "确认操作？", function () {
        $.post("/InStock/OtherInDepot/CKOtherDepot", { id: id }, function (data) {      
            if (data == "ck_yes") {
...
//获取ID
function getid() {
    $.post("/InStock/OtherInDepot/GetOtherID", {}, function (data) {
        if (data) {
            $("#OIDID").val(data);
        } else {
            $("#OIDID").val("RK201501010001");
        }
    });
}