...
$(function () {
    ...
    $("#probodyed").delegate("tr", "click", function () {
        ...
        $.post("/Stock/StockBreakage/GetDeteilByID", { id: stockid }, function (data) {
            if (data) {
                $("#stockbodyed tr").remove();
                $(data).each(function (k, v) {
                    var tr = $("<tr></tr>");
                    var td1 = $("<td>" + v.ProName + "</td>");
                    var td2 = $("<td>" + v.LDPrice + "</td>");
                    var td3 = $("<td>" + v.LDAmount + "</td>");
                    var td4 = $("<td><i style='color:green;cursor:pointer;font-size:16px;' class='am-icon-comment'  title='" + v.LDDesc + "'></i></td>");
    ...
    $("#btn_addorder").click(function () {
        ...
        var LostDesc = $("#LostDesc").text();
        $("#orderbody tr").each(function (k, v) {
            var tdid = $(v).children("td")[0].innerHTML;
            if (!isNaN(tdid)) {
                var LDAmount = $($(v).children("td")[5]).children("input")[0].value;//数量
                var LDPrice = $($(v).children("td")[6]).children("input")[0].value;//价格
                var LDDesc = $($(v).children("td")[8]).children("input")[0].value;//描述
                var st = { LostID: LostID, ProID: tdid, LDAmount: LDAmount, LDPrice: LDPrice, LDDesc: LDDesc };
                list.push(st);
            }
        });
        var stock = { LostID: LostID, DepotID: DepotID, LostDate: LostDate, LostState: 0, LostDesc: LostDesc };
        //alert(JSON.stringify(list));
        //alert(JSON.stringify(stock));
        var type = $(this).val();
        if (type == "保存") {
            $.post("/Stock/StockBreakage/Add", { pd: stock, list: list }, function (data) {
                if (data == "add_yes") {
            ...
            $.post("/Stock/StockBreakage/Edit", { pd: stock, list: list }, function (data) {
                if (data == "edit_yes") {
...
///加载采购订单数据【分页】
function Page() {
    var LostID = $("#LostID1").val();
    var DepotID = $("#DepotID1 option:selected").val();
    var LostDate = $("#LostDate1").val();
    var UsersName = $("#UsersName1").val();
    var LostState = $("input[name=LostState1]:checked").val();
    var obj = { LostID: LostID, UsersName: UsersName, DepotID: DepotID, LostDate: LostDate, LostState: LostState, PageIndex: PageIndex };
    // alert(JSON.stringify(obj));
    $.post("/Stock/StockBreakage/Find", obj, function (data) {
        if (data) {
...
function setPage(data) {
    ...
    $(data).each(function (k, v) {
        var tr = $("<tr></tr>");
        var td1 = $("<td>" + v.LostID + "</td>");
        var td2 = $("<td>" + v.DepotName + "</td>");
        var td3 = $("<td></td>");
        if (v.LostDate != "null" && v.LostDate != "undefined") {
            td3.text(ConvertTime(v.LostDate));
        }
        ...
        var td6 = $("<td ><i style='color:green;cursor:pointer;font-size:16px;' class='am-icon-comment'  title='" + v.LostDesc + "'></i></td>");
        var td7 = $("<td align='center'><button style='font-size:10px;' class='am-btn am-btn-warning am-radius   am-btn-xs '  " + (v.LostState == "1" ? "disabled='disabled'" : "") + "  onclick='orderedit(\"" + v.LostID + "\",\"" + v.DepotID + "\",\"" + v.UserID + "\",\"" + v.LostDate + "\",\"" + v.LostState + "\",\"" + v.LostDesc + "\")'><span class='am-icon-pencil-square-o'></span> 编辑</button><button style='font-size:10px;margin-left:3px;' class='am-btn am-btn-success am-radius am-btn-xs' " + (v.LostState == "1" ? "disabled='disabled'" : "") + "  onclick='order_ck(\"" + v.LostID + "\")' ><span class='am-icon-check'></span> 审核</button> <button style='font-size:10px;' class='am-btn am-btn-danger am-radius am-btn-xs' " + (v.LostState == "1" ? "disabled='disabled'" : "") + " onclick='order_del(\"" + v.LostID + "\")' ><span class='am-icon-trash-o'></span> 删除</button></td>");
...
function order_del(id) {
    new tmm_confirm("del", "提示", "确认要删除吗？", function () {
        $.post("/Stock/StockBreakage/Del", { id: id }, function (data) {
            if (data == "del_yes") {
...
//编辑入库单
function orderedit(LostID, DepotID, UserID, LostDate, LostState, LostDesc) {
    $("#btn_addorder").val("修改");
    ...
    $("#LostID").val(LostID);
    $("#LostDate").val(ConvertTime_(LostDate));
    $("#LostDesc").val(LostDesc);
    $("#DepotID option[value='" + DepotID + "']").prop("selected", true);
    //显示商品信息
    $.post("/Stock/StockBreakage/GetDeteilByID", { id: LostID }, function (data) {
        $("#orderbody tr").remove();
        if (data) {
            $(data).each(function (k, v) {
                var newtr = $('<tr><td align="center">' + v.ProID + '</td><td>' + v.ProName + '</td><td>' + v.PUName + '</td><td>' + v.PSName + '</td><td>' + v.PCName + '</td><td ><input  type="number" name="procount" value="' + v.LDAmount + '" placeholder="请输入商品数量" style="width:100px;font-size:11px;" required/></td><td><input type="number" value="' + v.LDPrice + '"  name="proprice"   placeholder="请输入商品单价" style="width:100px;font-size:11px;" required /></td><td>' + (v.LDAmount * v.LDPrice).toFixed(2) + '</td><td><input type="text" value="' + v.LDDesc + '" placeholder="备注"  style="width:160px;font-size:11px;"  /></td><td align="center"><a class="am-icon-remove" title="删除" href="#" style="font-size:18px;"></a></td></tr>');
                $("#orderbody").append(newtr);
            });
        }
    });
...
function order_ck(id) {
    new tmm_confirm("ck", "提示", "确认操作？", function () {
        $.post("/Stock/StockBreakage/CKOtherDepot", { id: id }, function (data) {
            if (data == "ck_yes") {
...
//获取ID
function getid() {
    $.post("/Stock/StockBreakage/GetID", {}, function (data) {
        if (data) {
            $("#LostID").val(data);
        } else {
            $("#LostID").val("BS201501010001");
        }
    });
}
//选择客户时
