...
$(function () {
    ...
    $("#probodyed").delegate("tr", "click", function () {
        ...
        $.post("/InOut/ProduceOutDepot/GetProduceOutDepotDetailByQPID", { id: stockid }, function (data) {
            if (data) {
                $("#stockbodyed tr").remove();
                $(data).each(function (k, v) {
                    var tr = $("<tr></tr>");
                    var td1 = $("<td>" + v.ProName + "</td>");
                    var td2 = $("<td>" + v.PODDPrice + "</td>");
                    var td3 = $("<td>" + v.PODDAmount + "</td>");
                    var td4 = $("<td><i style='color:green;cursor:pointer;font-size:16px;' class='am-icon-comment'  title='" + v.PODDDesc + "'></i></td>");
    ...
    $("#btn_addorder").click(function () {
        ...
        var PODDesc = $("#PODDesc").text();
        $("#orderbody tr").each(function (k, v) {
            var tdid = $(v).children("td")[0].innerHTML;
            if (!isNaN(tdid)) {
                var PODDAmount = $($(v).children("td")[5]).children("input")[0].value;//数量
                var PODDPrice = $($(v).children("td")[6]).children("input")[0].value;//价格
                var PODDDesc = $($(v).children("td")[8]).children("input")[0].value;//描述
                var st = { PODID: PODID, ProID: tdid, PODDAmount: PODDAmount, PODDPrice: PODDPrice, PODDDesc: PODDDesc };
                list.push(st);
            }
        });
        var stock = { PODID: PODID, DepotID: DepotID, PODDate: PODDate, PODState: 0, PODDesc: PODDesc };
        //alert(JSON.stringify(list));
        //alert(JSON.stringify(stock));
        var type = $(this).val();
        if (type == "保存") {
            $.post("/InOut/ProduceOutDepot/AddProduceOutDepot", { qp: stock, list: list }, function (data) {
                if (data == "add_yes") {
            ...
            $.post("/InOut/ProduceOutDepot/EditProduceOutDepot", { qp: stock, list: list }, function (data) {
                if (data == "edit_yes") {
...
///加载采购订单数据【分页】
function Page() {
    var PODID = $("#PODID1").val();
    var DepotID = $("#DepotID1 option:selected").val();
    var PODDate = $("#PODDate1").val();
    var UsersName = $("#UsersName1").val();
    var PODState = $("input[name=PODState1]:checked").val();
    var obj = { PODID: PODID, UsersName: UsersName, DepotID: DepotID, PODDate: PODDate, PODState: PODState, PageIndex: PageIndex };
   // alert(JSON.stringify(obj));
    $.post("/InOut/ProduceOutDepot/Find", obj, function (data) {
        if (data) {
...
function setPage(data) {
    ...
    $(data).each(function (k, v) {
        var tr = $("<tr></tr>");
        var td1 = $("<td>" + v.PODID + "</td>");
        var td2 = $("<td>" + v.DepotName + "</td>");
        var td3 = $("<td></td>");
        if (v.PODDate != "null" && v.PODDate != "undefined") {
            td3.text(ConvertTime(v.PODDate));
        }
        ...
        var td6 = $("<td ><i style='color:green;cursor:pointer;font-size:16px;' class='am-icon-comment'  title='" + v.PODDesc + "'></i></td>");
        var td7 = $("<td align='center'><button style='font-size:10px;' class='am-btn am-btn-warning am-radius   am-btn-xs '  " + (v.PODState == "1" ? "disabled='disabled'" : "") + "  onclick='orderedit(\"" + v.PODID + "\",\"" + v.DepotID + "\",\"" + v.UserID + "\",\"" + v.PODDate + "\",\"" + v.PODState + "\",\"" + v.PODDesc + "\")'><span class='am-icon-pencil-square-o'></span> 编辑</button><button style='font-size:10px;margin-left:3px;' class='am-btn am-btn-success am-radius am-btn-xs' " + (v.PODState == "1" ? "disabled='disabled'" : "") + "  onclick='order_ck(\"" + v.PODID + "\")' ><span class='am-icon-check'></span> 审核</button> <button style='font-size:10px;' class='am-btn am-btn-danger am-radius am-btn-xs' " + (v.PODState == "1" ? "disabled='disabled'" : "") + " onclick='order_del(\"" + v.PODID + "\")' ><span class='am-icon-trash-o'></span> 删除</button></td>");
...
function order_del(id) {
    new tmm_confirm("del", "提示", "确认要删除吗？", function () {
        $.post("/InOut/ProduceOutDepot/DelProduceOutDepot", { id: id }, function (data) {
            if (data == "del_yes") {
...
//编辑入库单
function orderedit(PODID, DepotID, UserID, PODDate, PODState, PODDesc) {
    $("#btn_addorder").val("修改");
    ...
    $("#PODID").val(PODID);
    $("#PODDate").val(ConvertTime_(PODDate));
    $("#PODDesc").val(PODDesc);
    $("#DepotID option[value='" + DepotID + "']").prop("selected", true);
    //显示商品信息
    $.post("/InOut/ProduceOutDepot/GetProduceOutDepotDetailByQPID", { id: PODID }, function (data) {
        $("#orderbody tr").remove();
        if (data) {
            $(data).each(function (k, v) {
                addCl = v.QPDDiscont;
                var newtr = $('<tr><td align="center">' + v.ProID + '</td><td>' + v.ProName + '</td><td>' + v.PUName + '</td><td>' + v.PSName + '</td><td>' + v.PCName + '</td><td ><input  type="number" name="procount" value="' + v.PODDAmount + '" placeholder="请输入商品数量" style="width:100px;font-size:11px;" required/></td><td><input type="number" value="' + v.PODDPrice + '"  name="proprice"   placeholder="请输入商品单价" style="width:100px;font-size:11px;" required /></td><td>￥' + v.PODDAmount * v.PODDPrice + '</td><td><input type="text" value="' + v.PODDDesc + '" placeholder="备注"  style="width:160px;font-size:11px;"  /></td><td align="center"><a class="am-icon-remove" title="删除" href="#" style="font-size:18px;"></a></td></tr>');
                $("#orderbody").append(newtr);
            });
        }
    });
...
function order_ck(id) {
    new tmm_confirm("ck", "提示", "确认操作？", function () {
        $.post("/InOut/ProduceOutDepot/CKProduceOutDepot", { id: id }, function (data) {
            if (data == "ck_yes") {
...
//获取ID
function getid() {
    $.post("/InOut/ProduceOutDepot/GetProduceOutDepotID", {}, function (data) {
        if (data) {
            $("#PODID").val(data);
        } else {
            $("#PODID").val("SL201501010001");
        }
    });
}
//选择客户时
